...
def mockexp_gleg_simSpec_fiducial(r_ap, iexp=562, seed=1): 
    ...
    # get source spectra  
    fsource = ''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/',
        'g15.source_spectra.r_ap', str(r_ap), '.fits'])
    ...
    f_sky = ''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/', 'sky_brightness.iexp', str(iexp), '.fits'])
    ...
    f_simspec = ''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/',
        'g15.sim_spectra.r_ap', str(r_ap), '.texp600.iexp', str(iexp), '.newsky.fits'])
    if not os.path.isfile(f_simspec):
...
def Redrock_mockexp_gleg_simSpec_fiducial(r_ap, iexp=562, ncpu=4): 
    ''' Run redrock 
    '''
    import redrock
    ...
    f_simspec = ''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/',
        'g15.sim_spectra.r_ap20.55.texp600.iexp', str(iexp), '.newsky.fits'])
    ...
    f_rrbest_tp = ''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/',
        'g15.sim_spectra.r_ap', str(r_ap), '.texp600.iexp', str(iexp), '.newsky.rr.best_tp.fits'])
    ...
    sub.set_ylim([0.5*zfit['chi2'].min(), 1.5*zfit['chi2'].max()])  
    ...
    fig.savefig(''.join([UT.dat_dir(), 'spectra/gamadr3_legacydr7/', 
        'g15.sim_spectra.r_ap', str(r_ap), '.texp600.iexp', str(iexp), '.newsky.rr.png']), bbox_inches='tight') 
    return None 


if __name__=='__main__': 
    #mockexp_gleg_simSpec_fiducial(20.55, iexp=562, seed=1)
    Redrock_mockexp_gleg_simSpec_fiducial(20.55, iexp=562, ncpu=4)
